// Downtone ethos reading document — Tweaks: color pairing + imagery density.
...
function ReadingTweaks() {
  const [t, setTweak] = useTweaks(RD_DEFAULTS);

  React.useEffect(() => {
    const [bg, accent] = t.pairing || RD_DEFAULTS.pairing;
    const root = document.documentElement;
    root.style.setProperty('--rd-bg', bg);
    root.style.setProperty('--rd-accent', accent);
  }, [t.pairing]);
...
(function mount() {
  ...
  if (!el) return;
  ReactDOM.createRoot(el).render(<ReadingTweaks />);
})();
